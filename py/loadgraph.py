...
if (len(sys.argv) > 1):
    ...
    for pair in itertools.combinations(robots, 2):
        #print("From: %d to %d:" % (pair[0].polygon_id, pair[1].polygon_id))
        ...
        if numWaypoints > 0:
            #print(shortest)
            builtstr = "%d,%d#" % (pair[0].polygon_id, pair[1].polygon_id)
            # First element
            builtstr += str(shortest[1].polygon_id)
            builtstr += ":"
            builtstr += str(shortest[1].polygon_id_2)
            
            # All the rest
            for waypoint in shortest[2:-1]:
                builtstr += "|"
                builtstr += str(waypoint.polygon_id)
                builtstr += ":"
                builtstr += str(waypoint.polygon_id_2)
        ...
        if (loopCounter % 20 == 0):
            ...
            completed = pair[0].polygon_id
            left = totalCount - pair[0].polygon_id
            if completed > 0:
            ...
            print("Status: %d:%d / %d. Elapsed: %f seconds. Estimated time remaining: %f s." % (completed, pair[1].polygon_id, totalCount, elapsed, time_remaining))
